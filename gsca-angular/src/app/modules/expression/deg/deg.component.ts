...
export class DegComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (!postTerm.validColl.length) {
      ...
      window.alert(
        'The "GSEA score", "Differential expression", and "Differential GSVA" are based on cancer types with sufficient paired tumor-normal samples (>= 10), including THCA, KIRP, BLCA, LIHC, HNSC, BRCA, LUAD, PRAD, ESCA, KICH, LUSC, KIRC, STAD and COAD. Please select at least one of these cancer types to access these analyses. Or you can explore "GSVA score" section or other sections, which provides gene expression analysis without a limitation on the paired tumor-normal sample size'
      );
    } else {
