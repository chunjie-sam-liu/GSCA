...
export class ImmuneSnvComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  displayedColumnsImmSnvCor = ['cancertype', 'symbol', 'cell_type', 'logfc', 'p_value', 'fdr'];
  displayedColumnsImmSnvCorHeader = ['Cancer type', 'Gene symbol', 'Cell type', 'Log2(FC)', 'P value', 'FDR'];
  ...
  immSnvCorSingleGeneImageLoading = false;
  ...
  public exportExcel() {
    const workSheet = XLSX.utils.json_to_sheet(this.dataSourceImmSnvCor.data, { header: this.displayedColumnsImmSnvCor });
    const workBook: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workBook, workSheet, 'SheetName');
    XLSX.writeFile(workBook, 'ImmuneAndSnvTable.xlsx');
  }
}
