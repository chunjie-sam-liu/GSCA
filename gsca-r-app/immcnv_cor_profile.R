...
fetched_data <- purrr::map(.x = search_colls, .f = fn_fetch_mongo, pattern="_immune_cor_cnv",fields = fields,.key=search_genes,.keyindex="symbol") %>%
  dplyr::bind_rows() %>%
...
title <- glue::glue("Correlation between CNV and immune\ninfiltrates in ",{search_cancertypes})
...
# Save --------------------------------------------------------------------
ggsave(filename = filepath, plot = plot, device = 'png', width = size$widt+2, height = size$height)
