...
filepath <- args[2]
apppath <- args[3]
# search_str<- "A2M#ACE#ANGPT2#BPI#CD1B#CDR1#EGR2#EGR3#HBEGF#HERPUD1#MCM2#PCTP#PODXL#PPY#PTGS2#RCAN1#SLC4A7#THBD@KICH_cnv_cor_expr#KIRC_cnv_cor_expr#KIRP_cnv_cor_expr#LUAD_cnv_cor_expr#LUSC_cnv_cor_expr"
...
fetched_data <- purrr::map(.x = search_colls, .f = fn_fetch_mongo, pattern="_cnv_cor_expr",fields = fields,.key=search_genes,.keyindex="symbol") %>%
  dplyr::bind_rows()
